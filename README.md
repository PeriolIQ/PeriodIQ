...
│  │    Cognito (JWT) ──→ JWT Authorizer                         │   │
│  │    (cấp JWT cho User + Admin)                               │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ 3. API LAYER                                                 │   │
...
|------|-------------|---------|
| Edge (CDN) | WAF (CLOUDFRONT), CloudFront, S3 | WAF chặn SQLi/XSS/bot + rate limit ở edge; CloudFront phân phối web tĩnh, định tuyến `/api` → API Gateway, HTTPS, OAC |
| Auth & Security | Cognito | Xác thực User & Admin, cấp JWT |
...
2. Request đi qua **WAF** (chặn bot/SQLi/XSS/rate limit ở edge) rồi vào CloudFront (CDN)
...
|---|---------|------|---------|
| 1 | AWS WAF | Edge (CDN) | Tường lửa ứng dụng web (WebACL scope CLOUDFRONT, gắn vào CloudFront distribution) |
| 2 | CloudFront | Edge (CDN) | CDN + định tuyến (được WAF bảo vệ) |
| 3 | S3 | Edge (CDN) | Hosting frontend SPA |
| 4 | Cognito | Auth & Security | Xác thực, cấp JWT |
...
>
> 📌 **WAF gắn vào CloudFront (bảo vệ ở tầng edge):**
> - WebACL dùng **scope `CLOUDFRONT`** (tạo ở region `us-east-1`) và **gắn trực tiếp vào CloudFront distribution** qua thuộc tính `WebACLId`.
> - Mọi request đi qua WAF trước khi vào CloudFront → chặn bot/SQLi/XSS + rate limit ngay ở edge, trước khi chạm S3 hoặc API Gateway.

| Dịch vụ AWS | Vai trò |
|-------------|---------|
| **Amazon Cognito** | Setup User Pool, cấu hình JWT cho User + Admin |
| **AWS WAF** | WebACL scope CLOUDFRONT (us-east-1), gắn vào CloudFront — chặn bot, SQL Injection, XSS, rate limit |
| **Amazon CloudFront** | Setup CDN, phân luồng static → S3 và /api → API Gateway; gắn WAF WebACL |
...
- **WAF rules** (WebACL scope **CLOUDFRONT**, ở `us-east-1`):
  - Tạo Web ACL và gắn vào **CloudFront distribution** qua thuộc tính `WebACLId` (scope CLOUDFRONT bắt buộc tạo ở us-east-1)
...
- **CloudFront distribution**:
  ...
  - **Gắn WAF WebACL** vào distribution qua `WebACLId` (xem mục WAF rules)
