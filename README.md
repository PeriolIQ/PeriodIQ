...
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ 1. EDGE (CDN)                                               │   │
│  │    User/Admin ──→ CloudFront ──┬→ S3 (React+Vite)          │   │
│  │                                └→ API Gateway               │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ 2. AUTH & SECURITY LAYER                                     │   │
│  │    WAF (REGIONAL) ──→ Cognito (JWT) ──→ JWT Authorizer      │   │
│  │    (WAF gắn vào Cognito User Pool: chặn bot/SQLi/XSS/rate)  │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ 3. API LAYER                                                 │   │
...
|------|-------------|---------|
| Edge (CDN) | CloudFront, S3 | Phân phối web tĩnh, định tuyến `/api` → API Gateway, HTTPS, OAC |
| Auth & Security | Cognito, WAF (REGIONAL) | Xác thực User & Admin, cấp JWT; WAF gắn vào Cognito User Pool chặn SQLi/XSS/bot + rate limit |
...
```
User ──→ CloudFront ──→ API Gateway ──→ JWT Authorizer (Cognito + WAF REGIONAL)
                                                        │
                                                        ▼
                                              Lambda · API Handler
                                                        │
...
2. Request đi qua CloudFront (CDN)
3. CloudFront route `/api` → API Gateway
4. API Gateway xác minh JWT (Cognito Authorizer — WAF REGIONAL bảo vệ Cognito, chặn bot/SQLi/XSS/rate limit), invoke Lambda
...
|---|---------|------|---------|
| 1 | AWS WAF | Auth & Security | Tường lửa ứng dụng web (WebACL REGIONAL, gắn Cognito User Pool) |
| 2 | CloudFront | Edge (CDN) | CDN + định tuyến |
| 3 | S3 | Edge (CDN) | Hosting frontend SPA |
| 4 | Cognito | Auth & Security | Xác thực, cấp JWT (được WAF bảo vệ) |
