...
import { FITNESS_LEVELS, GOALS } from '@/lib/constants';
...
const fieldCls =
  ...
  'focus:outline-none focus:ring-2 focus:ring-blue-400/40 focus:border-blue-400/60 transition-all appearance-none cursor-pointer';
  ...
  return (
    <Card className="col-span-1 md:col-span-12 p-6 shadow-sm">
      ...
      <div className="flex items-center gap-2 mb-5">
        <h2 className="text-xl font-bold">{t('profile.training_profile')}</h2>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <div className="space-y-1.5">
            <label className="flex items-center gap-1.5 font-mono text-xs font-bold text-muted-foreground uppercase tracking-wider">
              <Dumbbell className="w-3 h-3" /> {t('plan.level')}
            </label>
            ...
            {form.fitnessLevel && (
              <span className={`text-xs font-mono font-bold ${LEVEL_COLOR[form.fitnessLevel]}`}>
                â— {form.fitnessLevel.toUpperCase()}
              </span>
            )}
          </div>
          ...
          <div className="space-y-1.5">
            <label className="flex items-center gap-1.5 font-mono text-xs font-bold text-muted-foreground uppercase tracking-wider">
              <Target className="w-3 h-3" /> {t('plan.goal')}
            </label>
            <select className={fieldCls} value={form.primaryGoal}
              onChange={(e) => setField('primaryGoal', e.target.value)}>
              {GOALS.filter(g => ['Strength', 'Hypertrophy', 'Endurance', 'Weight Loss', 'General Fitness'].includes(g.value)).map((g) => <option key={g.value} value={g.value}>{t(g.labelKey)}</option>)}
            </select>
          </div>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="font-black text-2xl text-blue-400">{days}</span>
          </div>
          ...
          <div className="flex justify-between">
            {[1,2,3,4,5,6,7].map((n) => (
              <span key={n} className={`font-mono text-xs font-bold w-6 text-center
                ${n === days ? 'text-blue-400' : 'text-muted-foreground/40'}`}>
                {n}
              </span>
            ))}
          </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-muted/50 p-4 rounded-lg border border-border">
            ...
            <div className={`text-xl font-black ${LEVEL_COLOR[form.fitnessLevel] ?? 'text-foreground'}`}>
              {t(FITNESS_LEVELS.find(l => l.value === form.fitnessLevel)?.labelKey) || 'â€”'}
            </div>
          </div>
          <div className="bg-muted/50 p-4 rounded-lg border border-border">
            <div className="font-mono text-xs font-bold text-muted-foreground mb-1">GOAL</div>
            <div className="text-xl font-black text-purple-400">{t(GOALS.find(g => g.value === form.primaryGoal)?.labelKey) || 'â€”'}</div>
          </div>
          <div className="bg-muted/50 p-4 rounded-lg border border-border">
            <div className="font-mono text-xs font-bold text-muted-foreground mb-1">{t('profile.days_wk_short')}</div>
            <div className="text-xl font-black text-blue-400">{days}</div>
          </div>
        </div>
      </div>
    </Card>
